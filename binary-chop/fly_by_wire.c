...
klee_make_symbolic(&time_since_last_mega128, sizeof(unsigned char), "time_since_last_mega128");
klee_make_symbolic(&time_since_last_ppm, sizeof(unsigned short), "time_since_last_ppm");
...
klee_make_symbolic(&radio_really_lost, sizeof(unsigned char), "radio_really_lost");
...
klee_make_symbolic(&_20Hz, sizeof(unsigned char), "_20Hz");
	...
	//fbw_schedule();
        if (time_since_last_mega128 < STALLED_TIME){
                time_since_last_mega128++;wcet += 1;}
        if (time_since_last_ppm < REALLY_STALLED_TIME){
                time_since_last_ppm++;wcet += 1;}
        if (_1Hz == 0) {
	...
	if( ppm_valid )
	{
		...
		klee_make_symbolic(&tmp_radio, sizeof(signed short), "tmp_radio");
  		...
  		if (last_radio[RADIO_THROTTLE] > MAX_PPRZ) {
			last_radio[RADIO_THROTTLE] = MAX_PPRZ;wcet += 1;}
 		else if (last_radio[RADIO_THROTTLE] < 0) {
			last_radio[RADIO_THROTTLE] = 0; wcet += 1;}

  		tmp_radio = ppm_pulses[0] - (CLOCK*1600);
  		last_radio[0] = tmp_radio;
  		wcet += 1;
  		if (last_radio[0] > MAX_PPRZ) {
			last_radio[0] = MAX_PPRZ;wcet += 1;}
 		else if (last_radio[0] < MIN_PPRZ) {
			last_radio[0] = MIN_PPRZ;wcet += 1;} 
		...
		wcet += 11;
  		...
  		if (avg_cpt == AVERAGING_PERIOD) {
			...
			if(last_radio[RADIO_MODE]<TRESHOLD_MANUAL_PPRZ){
	        		mode = MODE_MANUAL;wcet += 1;}
			else{
	        		mode = MODE_AUTO;wcet += 1;}
      		}
      		if (mode == MODE_MANUAL)
      		{
  		...
  		//	servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_MOTOR_LEFT + (signed short)((2 * last_radio[RADIO_GAIN1])*SERVO_MOTOR_LEFT_TRAVEL);
  			servo_widths[0] = servo_value;
			wcet += 2;
			
  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_MOTOR_RIGHT + (signed short)((2 * last_radio[RADIO_THROTTLE])*SERVO_MOTOR_RIGHT_TRAVEL);
  		//	servo_widths[SERVO_MOTOR_RIGHT] = servo_value;
			wcet += 2;
			
  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_ELEVATOR + (signed short)((last_radio[0])*SERVO_ELEVATOR_TRAVEL);
  		//	servo_widths[SERVO_ELEVATOR] = servo_value;
			wcet += 2;
			
  			_var_roll = last_radio[0];
  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_AILERON_LEFT + (signed short)(((AILERON_DIFF) * _var_roll)*SERVO_AILERON_LEFT_TRAVEL);
  			servo_widths[SERVO_AILERON_LEFT] = servo_value;
			wcet += 3;
			
  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_AILERON_RIGHT + (signed short)(((AILERON_DIFF) * _var_roll)*SERVO_AILERON_RIGHT_TRAVEL);
  			servo_widths[0] = servo_value;
			wcet += 2;

  			//servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_RUDDER + (signed short)((last_radio[RADIO_YAW] + last_radio[RADIO_ROLL]*COMBI_SWITCH)*SERVO_RUDDER_TRAVEL);
		...
		if (mega128_receive_valid)
		{
			...
			if (mode == MODE_AUTO){
				...
				klee_make_symbolic(&_var_roll, sizeof(signed short), "_var_roll");
	  			...
	  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_MOTOR_LEFT + (signed short)((2 * from_mega128.channels[RADIO_GAIN1])*SERVO_MOTOR_LEFT_TRAVEL);
	  			servo_widths[SERVO_MOTOR_LEFT] = servo_value;
				wcet += 2;	  			

	  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_MOTOR_RIGHT + (signed short)((2 * from_mega128.channels[RADIO_THROTTLE])*SERVO_MOTOR_RIGHT_TRAVEL);
	  			servo_widths[SERVO_MOTOR_RIGHT] = servo_value;
				wcet += 2;	  				  			

	  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_ELEVATOR + (signed short)((from_mega128.channels[RADIO_PITCH])*SERVO_ELEVATOR_TRAVEL);
	  			servo_widths[SERVO_ELEVATOR] = servo_value;
				wcet += 2;	  				  			
				...
				wcet += 2;	  				  	
	  			...
	  			servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_RUDDER + (signed short)((from_mega128.channels[RADIO_YAW] + from_mega128.channels[RADIO_ROLL]*COMBI_SWITCH)*SERVO_RUDDER_TRAVEL);
	  			servo_widths[SERVO_RUDDER] = servo_value;
				wcet += 2;	  				  			
			}
	      }
	...
	if ( !(SPI_PIN>0 && SPI_SS_PIN>0) && spi_was_interrupted )
	{
		...
		unsigned char i;
  		for(i = 0; i < 9; i++){
     			to_mega128.channels[i] = last_radio[i];wcet += 1;}
		...
		(mode == MODE_AUTO && !mega128_ok))
	{
		...
		//servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_MOTOR_LEFT + (signed short)((2 * failsafe[RADIO_GAIN1])*SERVO_MOTOR_LEFT_TRAVEL);
  		//servo_widths[SERVO_MOTOR_LEFT] = servo_value;
  		wcet += 2;

		servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_MOTOR_RIGHT + (signed short)((2 * failsafe[RADIO_THROTTLE])*SERVO_MOTOR_RIGHT_TRAVEL);
		//servo_widths[SERVO_MOTOR_RIGHT] = servo_value;
		wcet += 2;

	  	//servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_ELEVATOR + (signed short)((failsafe[RADIO_PITCH])*SERVO_ELEVATOR_TRAVEL);
	  	//servo_widths[SERVO_ELEVATOR] = servo_value;
	  	wcet += 2;
	  	...
	  	//servo_value = (unsigned short)SERVOS_NEUTRALS+SERVO_RUDDER + (signed short)((failsafe[RADIO_YAW] + failsafe[RADIO_ROLL]*COMBI_SWITCH)*SERVO_RUDDER_TRAVEL);
	  	//servo_widths[SERVO_RUDDER] = servo_value;
	  	wcet += 2;
	}
       if (_20Hz >= 3){
