...
/* MDH WCET BENCHMARK SUITE. File version $Id: compress.c,v 1.7 2005/12/21
 * 09:37:18 jgn Exp $ */
 ...
 *
 * Changes:
 * JG 2005/12/20: Changed declaration of maxmaxcode to a warning
...
compress() {
  ...
  probe:
    ...
    bool a = ((long)htabof(i) > 0);
    bool b = (++apsim_bound111 < in_count);
    if ((a) && (b))
      goto probe;
  nomatch:
    ...
    if (free_ent < maxmaxcode) {
      ...
      bool c = ((count_int)in_count >= checkpoint);
...
putbyte(char c) { *OutBuff++ = c; /* apsim_unknown comp_text_buffer */ }
...
#else
  klee_make_symbolic(orig_text_buffer, BUFFERSIZE * sizeof(char),
                     "orig_text_buffer");
#endif
