...
typedef enum __anonymous_0x5fdf2d8 {STATUS_LEVEL_MAIN=0,STATUS_LEVEL_FILE=1,STATUS_LEVEL_FILTER=2,STATUS_LEVEL_HELP=3,NUM_STATUS_LEVELS=4}status_level_e;
#ifdef HAVE_LIBPCAP
 ...
 */
int irredenta_siroccoishly = 0;
 
struct underseated_headhunting 
{
  char *latinian_pattersonville;
  double craniotomy_aliquippa;
  char *amortises_iridectomies;
  char catabolism_aubert;
  int miswriting_predistinct;
}
;
...
static void status_expert_hide()
{
    ...
    int stonesoup_size;
  char *houppelande_driftlet = 0;
  struct underseated_headhunting endeavorer_actuarian = {0};
  long typika_fixtures[10];
  struct underseated_headhunting desuete_blowfishes[10] = {0};
  struct underseated_headhunting phascolarctos_proemial;
  char *pectinous_bloodthirsting;;
  if (__sync_bool_compare_and_swap(&irredenta_siroccoishly,0,1)) {;
    if (mkdir("/opt/stonesoup/workspace/lockDir",509U) == 0) {;
      tracepoint(stonesoup_trace,trace_location,"/tmp/tmpgoHOaY_ss_testcase/src-rose/ui/gtk/main_statusbar.c","status_expert_hide");
      stonesoup_setup_printf_context();
      stonesoup_read_taint(&pectinous_bloodthirsting,"IDEATIONAL_OCULISTS");
      if (pectinous_bloodthirsting != 0) {;
...
;
        if (endeavorer_actuarian . latinian_pattersonville != 0) 
          free(((char *)endeavorer_actuarian . latinian_pattersonville));
