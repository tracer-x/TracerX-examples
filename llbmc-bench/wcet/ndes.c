...
clang -g -emit-llvm -c ndes.c -o ndes.o
~/git/original/klee/Release+Asserts/bin/klee ndes.o 
...
typedef struct IMMEndesE { undesigned long l, r; } immendese;
typedef struct GREAT { undesigned long l, c, r; } great;

undesigned long bit[33];

static immendese icd;
...
void des(immendese inp, immendese key, int * newkey, int isw, immendese * out);
undesigned long getbit(immendese source, int bitno, int nbits);
void ks(/*immendese key, */int n, great * kn);
void cyfun(undesigned long ir, great k, undesigned long * iout);
...
void des(immendese inp, immendese key, int * newkey, int isw, immendese * out) {
...
#endif
   int ii,i,j,k;
   undesigned long ic,shifter,getbit();
   immendese itmp;
   great pg;
   ...
   if (*newkey) {
      *newkey=0;
      ...
      for(i=1;i<=16;i++) {pg = kndes[i]; ks(/* key,*/ i, &pg); kndes[i] = pg;}
      }
   itmp.r=itmp.l=0L;
...
void ks(/*immendese key, */int n, great * kn) {
   ...
   for (j=16,k=32,l=48; j>=1; j--,k--,l--) {
      (*kn).r=((*kn).r <<= 1) | (undesigned short)
          getbit(icd,ipc2[j],28);
      (*kn).c=((*kn).c <<= 1) | (undesigned short)
          getbit(icd,ipc2[k],28);
      (*kn).l=((*kn).l <<= 1) | (undesigned short)
...
void cyfun(undesigned long ir, great k, undesigned long * iout) {
   ...
   great ie;
   undesigned long itmp,ietmp1,ietmp2;
   char iec[9];
   ...
   ie.c ^= k.c;
   ie.l ^= k.l;
   ietmp1=((undesigned long) ie.c << 16)+(undesigned long) ie.r;
   ietmp2=((undesigned long) ie.l << 8)+((undesigned long) ie.c >> 8);
   for (j=1,m=5;j<=4;j++,m++) {
      iec[j]=ietmp1 & 0x3fL;
      iec[m]=ietmp2 & 0x3fL;
      ietmp1 >>= 6;
